...
{
    Task InitJobAsync(Guid jobId, int totalTiles, Guid userId, int pxFormat, byte opacity, int style, Guid? collectionId = null, CancellationToken ct = default);
...
public record JobInfo(Guid JobId, int TotalTiles, int CompletedTiles, Guid UserId, int PxFormat, byte Opacity, int Style, Guid? CollectionId, string Status);
